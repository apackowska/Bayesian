...
genMCMC = function( datFrm ,yName="y" , xNomName="xNom" , xMetName="xMet" ,
  ...
  xMet1 = as.numeric(datFrm[,xMetName])
  xMet = log(xMet1)
  Ntotal = length(y)
  NxLvl = length(unique(xNom))
  ...
  model {
    for ( i in 1:Ntotal ) {
      y[i] ~ dt(  a0 + a[xNom[i]] + aMet*( xMet[i] - xMetMean ), 1/ySigma[xNom[i]]^2 , nu )
    }
    nu ~  dexp(1/30.0) 
    for ( j in 1:NxLvl ) { 
    ...
    b0 <- a0 + mean( a[1:NxLvl] ) + aMet*(-xMetMean)
    for ( j in 1:NxLvl ) { b[j] <- a[j] - mean( a[1:NxLvl] ) }
  }
...
smryMCMC = function(  codaSamples , datFrm ,yName="y" , xNomName="xNom" , xMetName="xMet" ,
                      contrasts=NULL , saveName=NULL ) {
  # All single parameters:
  parameterNames = varnames(codaSamples) 
  if ( !is.null(datFrm) & !is.null(xNomName) ) {
    xlevels = levels(as.factor(datFrm[,xNomName]))
  }
  summaryInfo = NULL
...
plotMCMC = function( codaSamples , 
                     datFrm ,yName="y" , xNomName="xNom", contrasts=NULL ,
  ...
  plot(-1,0, 
       ...
       xlab=xNomName , xaxt="n" , ylab=yName ,
